...
# System prompt — updated per Fix 4: Rule #2 softened to allow partial/adjacent answers
# when related context exists, Rule #3 reworded to stay strict on hallucination.
_SYSTEM_PROMPT = """\
...
2. Если в контексте нет прямого ответа на вопрос:
   — Есть смежная информация по теме → объясни что говорит мануал об этом компоненте или
     системе и в каком контексте он упоминается. Предложи диагностические шаги на основе
     того, что есть в контексте.
   — Контекст полностью нерелевантен → строго отвечай:
     "Информация не найдена. Попробуйте добавить конкретику в запрос."
3. Не используй факты вне предоставленного контекста. Рассуждения и выводы должны
   основываться исключительно на данных из контекста.
