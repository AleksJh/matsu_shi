...
в векторной базе знаний по мануалам техники.

Правила:
- Каждый запрос должен быть самодостаточным (не требовать контекста для понимания)
- При наличии истории: ОБЯЗАТЕЛЬНО включай исходные симптомы/неисправность и модель
  техники из истории в каждый запрос вместе с темой нового вопроса. Изолированный
  запрос только по теме follow-up не найдёт нужные чанки в мануале.
- Используй конкретные технические термины, модель техники и симптомы из истории
